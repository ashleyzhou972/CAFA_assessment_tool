# -*- coding: utf-8 -*-
"""
Created on Wed Jun 21 17:22:38 2017
This for running big prediction files
Two modes and two types were run at the same time
(As it should be)
...
if __name__=='__main__':
    ...
    model = info[1]
    keywords = info[2][0]        
    resulthandle= open("./bigresults/%s_results.txt" % (os.path.basename(f.name).split('.')[0]),'w')
    prhandle = open("./pr_rc/%s_prrc.txt" % (os.path.basename(f.name).split('.')[0]),'w')
