...
metric = 'AUC'
taxon = '7227'
taxon_name = 'Drosophila'
go = '0007616'
go_name = 'long-term memory'
...
agg[bsl_indices,]
bsl_indices=c(97,112,68,23)
indices = c(teams_indices, bsl_indices)
cols = rep("#999999",length(teams_indices))
cols=c(cols, rep(c('#C4302B','#00539F'), each=length(bsl_indices)/2)) #red for blast, blue for blastcomp, 
...
text(x = xx, y = agg[indices,"metric"]-0.08, label = round(agg[indices,"metric"],2), 
...
abline(h = agg$metric[bsl_indices[4]], col = cols[8], lwd = 2, lty = 2)
dev.off()
